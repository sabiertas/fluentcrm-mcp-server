  ...
  return {
    tools: [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            custom_values: { type: 'object', description: 'Campos personalizados: { slug_del_campo: valor }. Los slugs deben existir en FluentCRM (Settings → Custom Fields).' },
          },
          required: ['email'],
        },
      },
      {
        ...
        description: 'Actualiza los datos de un contacto en FluentCRM, incluidos campos personalizados.',
        inputSchema: {
          type: 'object',
          properties: {
            site: siteProp,
            subscriberId: { type: 'number', description: 'ID del contacto' },
            first_name: { type: 'string', description: 'Nombre' },
            last_name: { type: 'string', description: 'Apellido' },
            phone: { type: 'string', description: 'Teléfono' },
            address_line_1: { type: 'string', description: 'Dirección' },
            city: { type: 'string', description: 'Ciudad' },
            country: { type: 'string', description: 'País (código ISO, ej: ES)' },
            status: { type: 'string', description: 'Estado', enum: ['subscribed', 'pending', 'unsubscribed'] },
            contact_type: { type: 'string', description: 'Tipo', enum: ['lead', 'customer'] },
            company_id: { type: 'string', description: 'Nombre de la empresa' },
            custom_values: { type: 'object', description: 'Campos personalizados: { slug_del_campo: valor }. Los slugs deben existir en FluentCRM (Settings → Custom Fields).' },
          },
          required: ['subscriberId'],
